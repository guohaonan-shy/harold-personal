...
function AsciiAvatarCard({ bootStage, bootScale = 1 }: AsciiAvatarCardProps) {
  ...
  const [mode, setMode] = useState<AvatarMode>("static");
  ...
  // 初始渲染与 SSR 一致(static),客户端确认细指针 + 非 reduced-motion 后才升级,
  // 因此粗指针 / reduced-motion 访客不会创建 WebGL 上下文
  useEffect(() => {
    if (
